...
class PluginManager
{
    ...
    public function discover(): Collection
    {
        ...
        foreach ($directories as $directory) {
            ...
            if ($plugin) {
                // Ensure the directory name matches the plugin ID
    ...
    public function installDependencies(string $pluginId): bool
    {
        ...
        if (! is_dir($pluginPath.'/vendor')) {
            mkdir($pluginPath.'/vendor', 0755, true);
        }
        ...
        if ($status !== 0) {
            $this->output->error('Failed to install dependencies: '.implode("\n", $output));
    ...
    /**
     * Enable a plugin by updating its plugin.json
     */
    /**
    ...
    /**
     * Disable a plugin by updating its plugin.json
     */
    /**
    ...
    public function isPluginEnabled(string $pluginId): bool
    {
        ...
        return $manifest['enabled'] ?? true; // Default to enabled if not set
    }
